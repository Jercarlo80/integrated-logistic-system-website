...
// Ikon (sama persis dengan Identifikasi)
const SearchIcon = () => (
  <svg
    className="h-5 w-5"
    viewBox="0 0 24 24"
...
const API_URL = "/api/v1/klasifikasi";
...
interface KlasifikasiProps {
  token?: string;
  apiUrl?: string;
  withCredentials?: boolean;
  /** Single selection: callback ketika item dipilih */
  onSelect?: (item: KlasifikasiItem | null) => void;
  /** Placeholder teks pencarian */
  placeholder?: string;
  /** Label di atas komponen */
  label?: string;
  /** Nilai yang dipilih (untuk controlled component) */
  selectedValue?: KlasifikasiItem | null;
}

// Helper functions (sama seperti sebelumnya, hanya menyesuaikan field 'uraian')
...
function getKode(item: Record<string, unknown>): string {
  return pick(item, ["kode", "code", "kode_klasifikasi", "id"]);
}
function getUraian(item: Record<string, unknown>): string {
  return pick(item, [
    "uraian",
    "detail",
    "nama",
...
function normalize(json: unknown): Record<string, unknown>[] {
  ...
  if (json && typeof json === "object") {
    ...
    if (getKode(obj) || getUraian(obj)) return [obj];
  }
  return [];
}
...
function buildSearchUrl(base: string, search: string): string {
  const q = search.trim();
  if (!q) return base;
  const param = isKodeQuery(q) ? "kode" : "uraian";
  return `${base}?${param}=${encodeURIComponent(q)}`;
}

// Depth styling (sama persis dengan Identifikasi: dot menggunakan bg-gray-950 + border)
const DEPTH_STYLES = [
  {
    dot: "bg-gray-950",
...
export default function Klasifikasi({
  ...
  onSelect,
  placeholder = "Cari kode (1.1.1) atau uraian (BAHAN)...",
  label = "Klasifikasi",
  ...
  const [selected, setSelected] = useState<KlasifikasiItem | null>(
    externalSelected ?? null,
  );

  // Sinkronisasi dengan prop selectedValue
  useEffect(() => {
  ...
  const fetchData = useCallback(
    async (search: string) => {
      if (!open) return; // hanya fetch jika dropdown terbuka
      setLoading(true);
      setError(null);
      try {
        ...
        if (res.status === 401)
          throw new Error("401 Unauthorized — token tidak ada / kedaluwarsa.");
        if (res.status === 503)
          throw new Error("Semua server API sedang tidak dapat dihubungi.");
        if (!res.ok) throw new Error(`Gagal memuat data (HTTP ${res.status})`);

        const json = await res.json();
        const mapped: KlasifikasiItem[] = normalize(json).map((raw) => {
          const kode = getKode(raw);
          return {
            kode,
            uraian: getUraian(raw),
            depth: getDepth(raw, kode),
            raw,
          };
        });
        setItems(mapped);
    ...
    [apiUrl, authToken, withCredentials, open],
  );
  ...
  const handleSelect = (item: KlasifikasiItem) => {
    setSelected(item);
    setQuery("");
    setOpen(false);
    onSelect?.(item);
  ...
  return (
    <div className="w-full font-sans">
      {/* Label dengan gaya uppercase seperti "JENIS BARANG" */}
      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">
        {label}
      </label>

      <div className="relative">
        {/* Input area + selected chip */}
        <div
        ...
        >
          {selected ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 pl-2.5 pr-1 py-0.5 text-sm font-medium text-emerald-300">
              ...
              <span className="max-w-[150px] truncate">{selected.uraian}</span>
              <button
                onClick={handleClear}
          ...
          <input
            ...
            aria-label="Pencarian klasifikasi"
          />
        ...
        {/* Dropdown panel hasil pencarian */}
        {open && (
              ...
              <ul className="py-1">
                {filtered.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`h-2 w-2 shrink-0 rounded-full ${style.dot} border border-white/20`}
                      />
      ...
      {isExpired && (
        <p className="mt-2 text-xs text-amber-400">
          ⚠️ Sesi habis, login ulang untuk mencari.
        </p>
      )}
    </div>
  );
